...
#
# Input:  bm/data/rds/mac_update3_best.rds  (required)
#         T_cell.rds (optional – section 3 T cell portion added when present)
# Output: bm/figures/update4/
# ============================================================
...
if (!requireNamespace("presto", quietly = TRUE)) {
  message("Installing presto for fast FindMarkers ...")
...
TCELL_RDS  <- "bm/data/raw/geo/integrated_Seurat_objects/47.integrated_object_subset_by_major_celltypes/T_cell.rds"
OUT_DIR    <- "bm/figures/update4"
...

{
  ...
  # Identify start cluster (most Mono-like archetype, or largest cluster as fallback)
  if ("archetype" %in% colnames(mac_bc_exp@meta.data)) {
    ...
    start_clust <- mac_bc_exp@meta.data %>%
      ...
      pull(mac_subcluster) %>%
      as.character()
    cat("  'archetype' column not found — using largest cluster as start:", start_clust, "\n")
  }
